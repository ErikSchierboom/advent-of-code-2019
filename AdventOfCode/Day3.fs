...
let parseDirection (instruction: string) =
...
let parseDistance (instruction: string) =
    int instruction.[1..]

let parseInstruction (instruction: string) =
    { Direction = parseDirection instruction
      Distance = parseDistance instruction }
    
let firstWireInstructions =
...
let applyInstruction current instruction =
    match instruction.Direction with
    | Left  ->
        [current.X - 1 .. -1 .. current.X - instruction.Distance]
        |> List.map (fun x -> { X = x; Y = current.Y })        
    | Right ->
        [current.X + 1 .. current.X + instruction.Distance]
        |> List.map (fun x -> { X = x; Y = current.Y })
    | Up ->
        [current.Y - 1 .. -1 .. current.Y - instruction.Distance]
        |> List.map (fun y -> { X = current.X; Y = y })
    | Down ->
        [current.Y + 1 .. current.Y + instruction.Distance]
        |> List.map (fun y -> { X = current.X; Y = y })
    
let pointsOnLine first second =
    if first.Y = second.Y then
        let start = min first.X second.X
        let stop = max first.X second.X
        [start + 1..stop] |> List.map (fun x -> { X = x; Y = first.Y })
    else
        let start = min first.Y second.Y
        let stop = max first.Y second.Y
        [start + 1..stop] |> List.map (fun y -> { X = first.X; Y = y })

let wirePoints instructions =
    let folder (points, current) instruction =
        let pointsForInstruction = applyInstruction current instruction
        (List.append points pointsForInstruction, List.last pointsForInstruction)

    List.fold folder ([], origin) instructions
    |> fst
...
let partOne =
    let firstWirePoints = wirePoints firstWireInstructions
    let secondWirePoints = wirePoints secondWireInstructions
    
    let crossingPoints = Set.intersect (set firstWirePoints) (set secondWirePoints)
...
let partTwo =
    ...
    let steps crossing points = List.findIndex (fun x -> x = crossing) points + 1
    let stepsSum crossing = steps crossing firstWirePoints + steps crossing secondWirePoints 
